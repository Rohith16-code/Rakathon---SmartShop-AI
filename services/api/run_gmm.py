...
def get_opt():
    ...
    if not os.path.exists(opt.checkpoint):
        raise FileNotFoundError(f"Checkpoint not found at {opt.checkpoint}")
    if not os.path.exists(opt.data_root):
        raise FileNotFoundError(f"Data root not found at {opt.data_root}")
...
def main():
    ...
    for mode in modes:
        print(f'Run on {mode.upper()} data')
